#Include %A_ScriptDir%\..\mouse_gesture.ahk
...
if (MG_PluginMenuCount)
{
	...
	Menu, PluginMenu, Add,
	Menu, PluginMenu, Add, %MG_LngMenu012%, MG_OpenPluginsFolder
	Menu, %MG_MenuName%, Add, %MG_LngMenu011%, :PluginMenu
}
else
{
	Menu, %MG_MenuName%, Add, %MG_LngMenu012%, MG_OpenPluginsFolder
}
Menu, %MG_MenuName%, Add,
Menu, %MG_MenuName%, Add, %MG_LngMenu005%, MG_ChooseLanguage
Menu, %MG_MenuName%, Add, %MG_LngMenu006%, MG_ShowHelp
Menu, %MG_MenuName%, Add, %MG_LngMenu007%, MG_About
Menu, %MG_MenuName%, Add,
Menu, %MG_MenuName%, Add, %MG_LngMenu008%, MG_Reload
Menu, %MG_MenuName%, Add, %MG_LngMenu009%, MG_Exit
if (MG_TraySubmenu)
{
	if (MG_MenuParent) {
		Menu, %MG_MenuParent%, Add, %MG_LngMenu010%, :MGMenu
	}
	else if (A_ScriptName = "MouseGestureL.ahk") {
		Menu, Tray, NoStandard
		Menu, Tray, Add, %MG_LngMenu010%, :MGMenu
		Menu, Tray, Add,
		Menu, Tray, Standard
	}
}
...
MG_EditUser()
{
	...
	if(!FileExist(A_ScriptDir . "\Config\MG_User.ahk"))
	{
		...
		; %MG_LngOthers004%
		;MG_Tooltip_RB_U = %MG_LngOthers005%
...
MG_ShowAboutDlg()
{
	...
	Gui, MGW_About:Add, Text, w240 Center Section, MouseGestureL.ahk Version %MG_Version%
	Gui, MGW_About:Font
	Gui, MGW_About:Add, Text, x40 y+12, Copyright (C) 2007-2008 lukewarm
	Gui, MGW_About:Add, Text, x40 y+4,  Copyright (C) 2011-2013 Pyonkichi
	Gui, MGW_About:Font, Underline cBlue
	Gui, MGW_About:Add, Text, xs y+8 w240 Center gMGW_AboutGoWebsite, Project Home
	Gui, MGW_About:Font
  ...
  MGW_AboutGoWebsite:
	Run, http://code.google.com/p/mousegesturel/
  MGW_AboutGuiClose:
  MGW_AboutGuiEscape:
...
MG_TriggerDown(name)
{
	...
	if(!InStr(MG_CurTriggers,name . "_"))
	{
		...
		else if (MG_TriggerCount==1)
		{
			...
			MG_GetMousePosInfo()
			MG_TX := MG_TL := MG_TR := MG_X
			MG_TY := MG_TT := MG_TB := MG_Y
			MG_PrevGesture := ""
			MG_NaviPrstStr := ""
			if (MG_IsDisableObj.())
			{
...
MG_Gesture(trigger)
{
	...
	loop
	{
		...
		else if(!MG_TimedOut && ((powx:=(dx-px)**2)+(powy:=(dy-py)**2) > MG_Threshold**2))
		{
			...
			if (MG_8Dir)
			{
				if (orange > Abs(0.7853981634-Abs(ATan((dx-px)/(dy-py))))) {
					d2 := (dx>px) ? ((dy>py) ? 3 : 9) : ((dy>py) ? 1 : 7)
				}
				else {
					d2 := (powx>powy) ? ((dx>px) ? 6 : 4) : ((dy>py) ? 2 : 8)
				}
			}
			else
			{
				d2:= (powx>powy) ? ((dx>px) ? "R":"L") : ((dy>py) ? "D":"U")
			}
			...
			else
			{
				; Check Long Movement
				if (MG_8Dir) {
					fChanged := ((dx-px2)**2+(dy-py2)**2 > MG_LongThreshold**2)
				}
				else if (powx > powy) {
					fChanged := (Abs(dx-px2) > MG_LongThresholdX)
				}
				else {
					fChanged := (Abs(dy-py2) > MG_LongThresholdY)
				}
			}
...
;-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
;-------------------------------------------------------------------------------
; Get the control handle which has focus
;														Implemented by lukewarm
;-------------------------------------------------------------------------------
MG_GetFocus()
{
...
;			  1 = Origin point is gesture end position
...
MG_EnumChildProc(hWnd, lParam)
{
	...
	WinGet, uStyle, Style, ahk_id %hWnd%
	if (!(uStyle & 0x10000000)) {
		return true
	}
	uSbStyle := lParam ? 0x0001 : 0x0000
	if ((uStyle & 0x000f) == uSbStyle)
	{
		static prevL, prevT
		WinGetPos, tgL, tgT, tgW, tgH, ahk_id %MG_HCTL%
		WinGetPos, sbL, sbT, sbW, sbH, ahk_id %hWnd%
		if (lParam)
		{
			if ((sbL+sbW)<tgL || sbL>(tgL+tgW) || (sbT+sbH)<=tgT || sbT>=(tgT+tgH))
			{
				return true
			}
		...
		{
			if ((sbT+sbH)<tgT || sbT>(tgT+tgH) || (sbL+sbW)<=tgL || sbL>=(tgL+tgW))
			{
				return true
			}
...
MG_ActivatePrevWin()
{
	...
	Loop, %winList%
	{
		...
		szTitle := "ahk_id " . winList%A_Index%
		WinGetClass, szClass, %szTitle%
		if ((szClass = "Progman")
		||	(szClass = "WorkerW")
		||	(szClass = "Shell_TrayWnd")
		||	(szClass = "SearchPane")
		||	(szClass = "NativeHWNDHost"))
		{
			continue
		}
		WinGetPos,,,w, h, %szTitle%
		if (w==0 || h==0) {
			continue
		}
		WinGet, nStyle, Style, %szTitle%
		WinGet, nExStyle, ExStyle, %szTitle%
		if ((nStyle   & 0x08000000) || (nExStyle & 0x00080000)) {
			continue
		}
		if (nStyle & 0x20000000)
		{
			if (!szMinWin) {
				szMinWin := szTitle
			}
...
;														Modified by Pyonkichi
;-------------------------------------------------------------------------------
MG_RunAsUser(szTarget, szFolder="", szWindow="")
{
	global MG_UserName, MG_Password
	if (MG_UserName) {
		RunAs, %MG_UserName%, %MG_Password%
	}
	Run, %szTarget%, %szFolder%, % szWindow . " UseErrorLevel"
	RunAs
}
...
;														Implemented by Pyonkichi
;-------------------------------------------------------------------------------
MG_CreateFont(szFace, nSize, fBold=0)
{
...
MG_UpdateAdNavi()
{
	...
	{
		local hDC := DllCall("GetWindowDC", "Ptr",MG_ExNaviHwnd, "Ptr")
		MG_hFntBtn := MG_CreateFont(MG_AdNaviFont, MG_AdNaviSize, 1)
	...
	{
		local uStyle
		WinGet, uStyle, Style, ahk_id %MG_ExNaviHwnd%
		if (uStyle & 0x10000000) {
			Gui, MGW_ExNavi:Show, x%winX% y%winY% NA
		}
	}
...
MG_UpdateAdNavi1(hDC, winX, winY, szGesture)
{
	global
	local gesW, gesH
	MG_DrawGesture(hDC, 0, 0, szGesture, 0, 1, gesW, gesH)
	MG_ActionStr := ""
	...
	MG_SetCornerPos(winX, winY, winW, winH)
	Gui,MGW_ExNavi:Show, x%winX% y%winY% w%winW% h%winH% NA
	...
	MG_DrawGesture(hDC, MG_AdNaviPaddingL, MG_AdNaviPaddingT, szGesture)
	if (actW) {
...
{
	global
	local gesW:=0, gesH:=0, actW:=0, nGes:=0, szGes:=""
	local len := StrLen(szGesture)
	;...........................................................................
	...
	Loop, %Gesture_Count%
	{
		...
		Loop, Parse, Gesture_%A_Index%_Patterns, `n
		{
			if (InStr(A_LoopField, szGesture)==1)
			{
				...
				local dir1 := A_LoopField
				local dir2 := szGesture
				Loop, Parse, MG_BtnNames, _
				{
					if (A_LoopField) {
				...
				MG_DrawGesture(hDC, 0, 0, A_LoopField, 0, 1, tmpW, tmpH1)
				if (gesW < tmpW) {
					gesW := tmpW
				}
				MG_ActionStr := " : " . MG_ActionStr
	...
	if (nGes)
	{
		...
		Loop, Parse, szGes, `,
		{
			MG_DrawGesture(hDC, MG_AdNaviPaddingL, ptY, A_LoopField, len)
			DllCall("SetTextColor", "Ptr",hDC, "UInt",MG_AdNaviFG2)
...
;-------------------------------------------------------------------------------
; Set Corner Window Position
;														Implemented by Pyonkichi
;-------------------------------------------------------------------------------
MG_SetCornerPos(ByRef winX, ByRef winY, winW, winH)
{
	global
	if (MG_AdNaviPosition == 1)
	{
...
;														Implemented by Pyonkichi
;-------------------------------------------------------------------------------
MG_DrawGesture(hDC, ptX, ptY, szGesture, nGesLen=0, fMeasure=0, ByRef strW=0, ByRef strH=0)
{
	global MG_BtnNames, MG_AdNaviNI2, MG_hFntBtn, MG_hFntDir
	static DirD=0xEA, DirL=0xE7, DirR=0xE8, DirU=0xE9
	static Dir1=0xED, Dir2=0xEA, Dir3=0xEE, Dir4=0xE7, Dir6=0xE8, Dir7=0xEB, Dir8=0xE9, Dir9=0xEC

	hFntOld := DllCall("SelectObject", "Ptr",hDC, "Ptr",MG_hFntBtn, "Ptr")
	VarSetCapacity(size, 8, 0)
	pos:=1, preFont:=1, nowX:=ptX, strH:=0
	while (pos <= StrLen(szGesture))
	{
		...
		if (!fMeasure)
		{
			DllCall("TextOut", "Ptr",hDC, "Int",nowX, "Int",ptY, "Str",szDraw, "Int",StrLen(szDraw))
		}
